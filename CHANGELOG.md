...
- Undo and redo for feed edits: every mutation made through the GUI —
  stops, routes, trips, timetable cells, cascades like dropping a route
  or a trip — is one undoable step. Undo/Redo buttons on the map toolbar
  (editing mode) show what they would revert, Ctrl/Cmd+Z and
  Ctrl/Cmd+Shift+Z (or +Y) work while editing, and the current feed's
  next undo/redo labels ride on the feed summary (``POST /api/undo``,
  ``POST /api/redo``; a feed edited outside the log answers 409 rather
  than corrupting anything). Saving a feed from the GUI now writes the
  change-log sidecar ``<name>.changes.txt`` beside it — the editor's
  internal scratch and session-embed saves are excluded. Requires
  transitio >= 0.7.
